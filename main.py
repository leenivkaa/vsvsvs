...
class MainApp(ctk.CTk):
    def __init__(self):
        ...
        # Загрузка фона
        bg = ctk.CTkImage(Image.open("zxc.png"), size=(700, 400))
        ...
        references_menu.add_command(label="Поставщик", command=lambda: self.show_table("SELECT * FROM postavchik"))
        references_menu.add_command(label="Продажи", command=lambda: self.show_table("SELECT * FROM sale"))
        references_menu.add_command(label="Поступление", command=lambda: self.show_table("SELECT * FROM postuplenie"))
        self.menu_bar.add_cascade(label="Справочники", menu=references_menu)

        # Меню "Таблицы"
        tables_menu = tk.Menu(self.menu_bar, tearoff=0)
        tables_menu.add_command(label="Муз инструменты", command=lambda: self.show_table("SELECT * FROM muz_instr"))
        tables_menu.add_command(label="Вид муз инструментов", command=lambda: self.show_table("SELECT * FROM vid_muz_instr"))
